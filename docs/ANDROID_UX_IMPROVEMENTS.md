...
- [x] On the secondary bar, replace y/n buttons with tmux copy-mode controls — `49f9a93` — _(native: implemented as **local** scroll controls on the `KeyboardDownBar` rather than tmux copy-mode — the `terminal:` channel has no copy-mode handler and native scrollback is the emulator's own transcript. The bar gained a Scroll / Exit Scroll toggle plus ▲/▼ page keys; see §8 `84654fd` for the shared implementation.)_
...
- [x] Replace the Copy button with a Scroll / Exit-Scroll toggle — `84654fd` — _(native: the `KeyboardDownBar` gained a **Scroll / Exit Scroll** toggle. Native has no Copy button and no tmux copy-mode snapshot, but it has the web's key behavior — a paused live view: entering scroll calls `TerminalView.enterScrollback`, which sets the emulator's `setAutoScrollDisabled(true)` (the new setter beside `toggleAutoScrollDisabled`) so incoming output stops yanking the viewport to the bottom, then pages up; ▲/▼ page through the transcript via `pageScroll`; Exit Scroll resumes auto-scroll and jumps to the live bottom. Raising the IME auto-exits so you never type into a frozen view. Tested in `KeyboardDownBarTest`.)_
- [x] _(web-impl detail)_ Drive scrollback via the terminal emulator instead of tmux copy-mode — `0da0a49` _(native already has its own scrollback buffer; adopt the same UX, not the xterm.js mechanism)_ — _(satisfied: native scrollback is the terminal-lib `TerminalBuffer` transcript; nothing uses tmux copy-mode)_
