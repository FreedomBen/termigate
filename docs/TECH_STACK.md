...
| Layer | Choice | Notes |
|-------|--------|-------|
| Language | Elixir 1.19+ | Concurrency model ideal for streaming terminal I/O |
| Framework | Phoenix 1.8 | Latest stable. LiveView ~1.1 for real-time web UI, Channels for native app |
...
| CSS framework | Tailwind CSS 4 | CSS-first config, `@theme` directive |
...
| Auth | pbkdf2_elixir 2.x | PBKDF2-HMAC-SHA512 password hashing (+ optional `TERMIGATE_AUTH_TOKEN` env var) |
