...
**`server/lib/termigate/tmux/session.ex`** — Session struct:
```elixir
...
**`server/lib/termigate/tmux/pane.ex`** — Pane struct:
```elixir
defmodule Termigate.Tmux.Pane do
...
**`server/lib/termigate/tmux_manager.ex`** — stateless module (not a GenServer):
...
All tmux commands go through the `CommandRunnerBehaviour` implementation (resolved via `Application.get_env(:termigate, :command_runner)`).
...
**`server/lib/termigate/ring_buffer.ex`** — circular byte buffer:
...
```
server/lib/termigate/tmux/session.ex
server/lib/termigate/tmux/pane.ex
server/lib/termigate/tmux_manager.ex
server/lib/termigate/ring_buffer.ex
server/test/termigate/tmux_manager_test.exs
server/test/termigate/ring_buffer_test.exs
```
