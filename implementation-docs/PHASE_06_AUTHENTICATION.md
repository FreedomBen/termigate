...
**`server/lib/termigate/auth.ex`**:

- Credentials file: `~/.config/termigate/credentials` (format: `username:pbkdf2_hash`, single line — this is a single-user system by design)
- `verify_credentials(username, password)` → `:ok` or `:error`
  - Check `TERMIGATE_AUTH_TOKEN` first (via `Application.get_env(:termigate, :auth_token)`): if set and password matches (constant-time via `Plug.Crypto.secure_compare/2`), return `:ok`
...
**`server/lib/mix/tasks/termigate.setup.ex`** — `mix termigate.setup` (run from `server/`):
...
**`server/lib/mix/tasks/termigate.change_password.ex`** — `mix termigate.change_password` (run from `server/`):
...
**`server/lib/termigate_web/live/auth_live.ex`**:
...
**`server/lib/termigate_web/live/auth_live.html.heex`**:
...
**`server/lib/termigate_web/live/auth_hook.ex`**:
...
**`server/lib/termigate_web/plugs/require_auth.ex`**:
...
**`server/lib/termigate_web/plugs/require_auth_token.ex`**:
...
**`server/lib/termigate_web/controllers/auth_controller.ex`**:
...
**`server/lib/termigate_web/rate_limit_store.ex`** — GenServer:
...
**`server/lib/termigate_web/plugs/rate_limit.ex`** — Plug:
...
**`server/lib/termigate_web/channels/user_socket.ex`** — update the stub created in Phase 5:
...
In `server/lib/termigate/application.ex`: if the endpoint is bound to `0.0.0.0` and `Auth.auth_enabled?/0` is false, log a warning:
"WARNING: Listening on 0.0.0.0 with no authentication configured. Set up auth via `cd server && mix termigate.setup` or set TERMIGATE_AUTH_TOKEN."
...
```
server/lib/termigate/auth.ex
server/lib/mix/tasks/termigate.setup.ex
server/lib/mix/tasks/termigate.change_password.ex
server/lib/termigate_web/live/auth_live.ex
server/lib/termigate_web/live/auth_live.html.heex
server/lib/termigate_web/live/auth_hook.ex
server/lib/termigate_web/plugs/require_auth.ex
server/lib/termigate_web/plugs/require_auth_token.ex
server/lib/termigate_web/plugs/rate_limit.ex
server/lib/termigate_web/rate_limit_store.ex
server/lib/termigate_web/controllers/auth_controller.ex
server/lib/termigate_web/channels/user_socket.ex
server/lib/termigate_web/router.ex (update)
server/lib/termigate/application.ex (startup warning)
...
## Exit Criteria
- `cd server && mix termigate.setup` creates credentials file with bcrypt-hashed password
...
- `TERMIGATE_AUTH_TOKEN` env var works as password fallback
